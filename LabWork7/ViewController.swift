...
//  LabWork7
//
//  Created by user235217 on 3/16/24.
//
...
class ViewController: UIViewController, CLLocationManagerDelegate {
    ...
    func TripSummary() {
        ...
        for x in speed {
            let currentAcceleration = (x - lastSpeed) / (timetaken / Double(speed.count))
...
extension ViewController{
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        let tempSpeed = location.speed >= 0 ? location.speed : 0
        currentSpeed = tempSpeed
        maxSpeed = max(maxSpeed, currentSpeed)
        speed.append(currentSpeed)
        
        if let starttrp = startTripTime {
                timetaken = Date().timeIntervalSince(starttrp)
            }
        
        if let previousLocation = previousLocation {
            let timeInterval = location.timestamp.timeIntervalSince(previousLocation.timestamp)
            if timeInterval > 0 {
                let Acceleration = abs(tempSpeed - lastSpeed) / timeInterval
